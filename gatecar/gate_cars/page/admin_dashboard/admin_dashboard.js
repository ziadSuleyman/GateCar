...
function load_dashboard(page) {
	...
	container.html(
		'<div class="ad-loading"><i class="fa fa-spinner fa-spin fa-2x"></i><p style="margin-top:12px;">جارٍ التحميل…</p></div>',
	);
...
function render_dashboard(
	...
	const company_share = revenue_ex_tax * 0.2;
	// المستثمر: 80% من الإيراد (بعد استبعاد الضريبة) ناقص الصيانة الدورية والمصاريف العامة.
	const investor_share = revenue_ex_tax * 0.8 - (periodic + expense);
						...
						<div class="ad-select-row" onclick="event.stopPropagation()">
							<select id="profit-month" class="ad-select">
								...
								${[
									...
									.map(
										([v, l]) =>
											`<option value="${v}" ${filter_month == v ? "selected" : ""}>${v} - ${l}</option>`,
									)
									.join("")}
							</select>
